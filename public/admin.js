...
let currentState = null, currentDisplays = [], targetSlideshowScreenId = null, tempIndividualPlaylist = [], tempMirrorPlaylist = [], deferredInstallPrompt = null;
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js').catch(() => {});

window.addEventListener('beforeinstallprompt', (e) => {
  e.preventDefault(); deferredInstallPrompt = e;
  const btn = document.getElementById('btn-install-app'); if (btn) btn.style.display = 'inline-flex';
});
...
if (typeof listenFirebaseState === 'function') listenFirebaseState((s) => { if (s) { currentState = s; renderUI(); } });
...
function switchMode(mode) {
  if (currentState) { currentState.mode = mode; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); }
...
function handleUnpairTv(screenId, e) {
  ...
  if (confirm(`¿Desvincular TV ${screenId}?`)) { if (typeof unpairTv === 'function') unpairTv(screenId); socket.emit('clear-screens', screenId); }
}
...
function renderSplitView() {
  const conf = currentState.splitConfig; document.getElementById('split-fit').value = conf.fit || 'cover';
  const grid = document.getElementById('wall-matrix-grid'); grid.style.gridTemplateColumns = `repeat(${conf.cols}, 1fr)`; grid.innerHTML = '';
...
async function uploadMedia(file) {
  ...
  const res = await fetch('/api/upload-base64', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data: base64Data, filename: file.name, isVideo }) });
...
function openSlideshowModal(screenId) {
  ...
  const existing = currentState && currentState.screens ? currentState.screens[screenId] : null;
...
function submitIndividualSlideshow() {
  ...
  if (currentState && currentState.screens) {
...
document.getElementById('mirror-file-input').addEventListener('change', async (e) => {
  if (e.target.files.length > 0) {
    ...
    if (currentState) { currentState.mirrorConfig = { type, src: url, fit: 'contain' }; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); }
    socket.emit('update-mirror', { type, src: url });
  }
});
function sendMirrorUrl() {
  const s = document.getElementById('mirror-url-input').value.trim();
  if (s && currentState) { currentState.mirrorConfig = { type: 'url', src: s, fit: 'contain' }; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); socket.emit('update-mirror', { type: 'url', src: s }); }
}
function sendMirrorText() {
  const t = document.getElementById('mirror-text-input').value.trim();
  if (t && currentState) { currentState.mirrorConfig = { type: 'text', text: t, fit: 'contain' }; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); socket.emit('update-mirror', { type: 'text', text: t }); }
}
function clearMirrorMedia() {
  if (currentState) { currentState.mirrorConfig = { type: 'empty', src: null, items: [], text: '' }; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); socket.emit('update-mirror', { type: 'empty', src: null, items: [], text: '' }); }
}

document.getElementById('split-file-input').addEventListener('change', async (e) => {
  if (e.target.files.length > 0) {
    const { url, type } = await uploadMedia(e.target.files[0]);
    if (currentState) { currentState.splitConfig.type = type; currentState.splitConfig.src = url; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); socket.emit('update-split', { type, src: url }); }
  }
});
function clearSplitMedia() {
  if (currentState) { currentState.splitConfig.type = 'empty'; currentState.splitConfig.src = null; currentState.splitConfig.text = ''; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); socket.emit('update-split', { type: 'empty', src: null, text: '' }); }
}
function applySplitPreset() {
  ...
  if (currentState) { currentState.splitConfig.rows = rows; currentState.splitConfig.cols = cols; currentState.splitConfig.layout = layout; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); socket.emit('update-split', { rows, cols, layout }); }
}
function updateSplitConfig() {
  const fit = document.getElementById('split-fit').value;
  if (currentState) { currentState.splitConfig.fit = fit; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); socket.emit('update-split', { fit }); }
}

async function uploadScreenMedia(screenId, input) {
  if (input.files.length > 0) {
    const { url, type } = await uploadMedia(input.files[0]);
    if (currentState && currentState.screens) { currentState.screens[screenId] = { type, src: url, fit: 'contain' }; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); socket.emit('update-screen', { screenId, type, src: url }); }
  }
}
function promptScreenUrl(screenId) {
  const u = prompt('URL:');
  if (u && currentState && currentState.screens) { currentState.screens[screenId] = { type: 'url', src: u.trim(), fit: 'contain' }; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); socket.emit('update-screen', { screenId, type: 'url', src: u.trim() }); }
}
function promptScreenText(screenId) {
  const t = prompt('Mensaje:');
  if (t && currentState && currentState.screens) { currentState.screens[screenId] = { type: 'text', text: t.trim(), fit: 'contain' }; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); socket.emit('update-screen', { screenId, type: 'text', text: t.trim() }); }
}
function changeScreenFit(screenId, fit) {
  if (currentState && currentState.screens && currentState.screens[screenId]) { currentState.screens[screenId].fit = fit; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); socket.emit('update-screen', { screenId, fit }); }
}
function clearScreenMedia(screenId) {
  if (currentState && currentState.screens && currentState.screens[screenId]) { currentState.screens[screenId] = { type: 'empty', src: null, text: '' }; if (typeof updateFirebaseState === 'function') updateFirebaseState(currentState); socket.emit('clear-screens', screenId); }
}
...
async function submitPairPin() {
  ...
  if (!cleanPin || cleanPin.length !== 6) return alert('Ingresa los 6 dígitos del código PIN de la TV (Ej: 143860).');
  ...
  try {
    if (typeof pairTvWithPin === 'function') {
      ...
      socket.emit('pair-pin', { pin: cleanPin, targetScreenId }, (res) => {
        if (res.success) { alert(`¡Vinculada a TV ${res.screenId}!`); closePairModal(); if (pinInput) pinInput.value = ''; }
        else alert(res.message || 'Error al vincular');
      });
    }
  } catch (err) {
    console.error('Pair error:', err);
    alert('Error al vincular: ' + (err.message || 'No se pudo conectar a Firebase'));
  } finally {
